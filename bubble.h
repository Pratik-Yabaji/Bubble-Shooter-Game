...
const int BUBBLE_DEFAULT_RADIUS = 50;
...
public:
    ...
    void nextStep(double t)
    {
        ...
        double new_y = circle.getY() + vy*t  + a*t*t;
        ...
        // This is written to make the bubble move verticaly
        if ((vy < 0 && new_y < (20 + circle.getRadius())) // bounce along Y direction at left border
            ||
            (vy > 0 && new_y > ((WINDOW_Y - 50) - circle.getRadius()))) // bounce along Y direction at right border
        {

            vy = -vy ;
            new_y = circle.getY() + vy*t + a*t*t;
        }                                  // ACCELERATION FOR PARABOLLIC PATH 
